...
for iN=1:length(NTimes)
    yf_amp = vf.*amplify.^(NTimes(iN));
    P1_ampN = real(ifft(yf_amp));
%     P1_ampN = P1_ampN./max(P1_ampN(:));
    sound(P1_ampN,fs);
    figure(5); subplot(N,M,iN+1); semilogx(f_vec, getSingleSideMagnitudeSpectrum(yf_amp,fs));  title(['amplitude spectrum (after mic ' num2str(iN) ' times)']); ylabel('|Y(f)|'); axis([0,22300,0,inf]);
    figure(6); subplot(N,M,iN+1); plot(t_vec,P1_ampN); title(['y_{' num2str(iN) '}(t) (after mic ' num2str(iN) ' times)']);
    pause(l_y);
end
